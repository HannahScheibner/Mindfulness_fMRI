...
function [names, onsets_thought, durations] = MF_time_Model4(prefix, name)
...
durations_thought=cell(1,1);



for j= 1:length(fcode)
    if strcmp(fcode{j}, 'Ton Start')==1 
        save_time(i+1) = ((str2num(ftime{j})-ftime_start)/10000)+3;
    end
...
%                      onsets_thought{event_types(i)} = [onsets_thought{event_types(i)} (event_time(i)-4)]; %onset minus 4 sec
                     ...
                     if response_time(i) - event_time(i)  >3
                        ...
                        onsets_thought{event_types(i)} = [onsets_thought{event_types(i)} save_time(i)];
%                         onsets_nointerest{1} = [onsets_nointerest{1} (response_time(i))];
                        save_time(i+1) = (response_time(i)+6);
                     else
...
%                          onsets_nointerest{1} = [onsets_nointerest{1} (event_time(i)+3)];
                        onsets_thought{event_types(i)} = [onsets_thought{event_types(i)} save_time(i)];
                        save_time(i+1) = (event_time(i)+9);
                     end;
                     if i >1
%                         durations_nointerest{1} = [durations_nointerest{1} ((event_time(i)-4)-n(i-1))];
                            durations_thought{event_types(i)} = [durations_thought{event_types(i)} (event_time(i)- save_time(i)];
                        size_NI=size_NI+1;
                     end;
                     l = 1;
            end;
...
cd(subj_dir);
cd('onsets');
save(strcat(prefix,name,'_time_Model4.mat'), 'names', 'names_extras','onsets_thought', 'onsets_response', 'onsets_shift', 'onsets_focus', 'durations_thought', 'durations_response', 'durations_shift', 'durations_focus');
if computer ==1
