...
function [names, onsets_thought, durations] = MF_time_Model5(prefix, name)
...
for l=1:length(onsets_thought)
    durations_thought{l}=onsets_thought{l}*0+10;
end
...
cd(subj_dir);
cd('onsets');
save(strcat(prefix,name,'_time_Model5.mat'), 'names', 'onsets_thought',  'durations_thought');
if computer ==1
