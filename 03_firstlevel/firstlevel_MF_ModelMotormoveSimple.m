...
for g = 1:size(subs,2)
    ...
    if specify
        ...
        matlabbatch{z}.spm.stats.fmri_spec.timing.RT              = 2.25;
...
%                  matlabbatch{z}.spm.stats.fmri_spec.sess(s).scans             = select_scans('^s8.*',[base filesep num2str(subs(g)) filesep task filesep num2str(s)]) %kann je nach smoothing ge�ndert werden
            ...
            matlabbatch{z}.spm.stats.fmri_spec.sess(s).cond(1).name      = sess_onsets.names{1,1}; %Motor
...
%             matlabbatch{1}.spm.stats.fmri_spec.sess.multi_reg = {'F:\MF_MRTStudie\01_Daten\Probanden\100\NIFTI\MF_Task_0005\rp_fMF_w_HC_100-152603-00006-00006-1.txt'};
    ...
    % ---------
        if inference
            ...
            matlabbatch{z}.spm.stats.con.consess{1}.fcon.convec  = {[eye(28) zeros(28,1)]}; %% 28 = es gibt 7 Moglichkeiten fuer 4 sessions
            
            
            matlabbatch{z}.spm.stats.con.consess{2}.tcon.name    = 'Response';
            matlabbatch{z}.spm.stats.con.consess{2}.tcon.convec  = [1 zeros(1,6) 1 zeros(1,6) 1 zeros(1,6) 1 zeros(1,6)];
